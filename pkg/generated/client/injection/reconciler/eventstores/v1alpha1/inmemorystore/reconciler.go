...
import (
	...
	eventstoresv1alpha1 "github.com/triggermesh/eventstore/pkg/generated/client/listers/eventstores/v1alpha1"
	zap "go.uber.org/zap"
...
// controller reconciling v1alpha1.InMemoryStore.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile v1alpha1.InMemoryStore. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *v1alpha1.InMemoryStore) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.InMemoryStore.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize v1alpha1.InMemoryStore. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *v1alpha1.InMemoryStore) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling v1alpha1.InMemoryStore if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe v1alpha1.InMemoryStore.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *v1alpha1.InMemoryStore) reconciler.Event
}

// ReadOnlyFinalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing v1alpha1.InMemoryStore if they want to process tombstoned resources
...
type ReadOnlyFinalizer interface {
	// ObserveFinalizeKind implements custom logic to observe the final state of v1alpha1.InMemoryStore.
	// This method should not write to the API.
	ObserveFinalizeKind(ctx context.Context, o *v1alpha1.InMemoryStore) reconciler.Event
}

type doReconcile func(ctx context.Context, o *v1alpha1.InMemoryStore) reconciler.Event

// reconcilerImpl implements controller.Reconciler for v1alpha1.InMemoryStore resources.
type reconcilerImpl struct {
	...
	Lister eventstoresv1alpha1.InMemoryStoreLister
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client internalclientset.Interface, lister eventstoresv1alpha1.InMemoryStoreLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.InMemoryStores(s.namespace)
...
func (r *reconcilerImpl) updateStatus(ctx context.Context, existing *v1alpha1.InMemoryStore, desired *v1alpha1.InMemoryStore) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {

			getter := r.Client.EventstoresV1alpha1().InMemoryStores(desired.Namespace)
		...
		updater := r.Client.EventstoresV1alpha1().InMemoryStores(existing.Namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.InMemoryStore) (*v1alpha1.InMemoryStore, error) {

	getter := r.Lister.InMemoryStores(resource.Namespace)
	...
	patcher := r.Client.EventstoresV1alpha1().InMemoryStores(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *v1alpha1.InMemoryStore) (*v1alpha1.InMemoryStore, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *v1alpha1.InMemoryStore, reconcileEvent reconciler.Event) (*v1alpha1.InMemoryStore, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
